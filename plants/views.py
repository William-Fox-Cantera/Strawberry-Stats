...
def farm_view(request, pk):
    ...
    coords = [ coord.replace("\r", "") for coord in coords ] # Remove escape character generated by read()
    if coords[len(coords)-1] == "": # Remove last empty string that exists for some reason
        coords.pop()
    coords = [ coord.split(",", 1)[1] for coord in coords ] # Remove indices, no longer needed
    coords_list = []
    ...
    gps_coords_json = json.dumps(coords_list)

    context = { 'customer':customer, 'coordinates':gps_coords_json }
